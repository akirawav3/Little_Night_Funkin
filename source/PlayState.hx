...
class PlayState extends MusicBeatState
{
	...
	public static var noteSplashes:FlxTypedGroup<FlxSprite> = null;
	...
	var bgSpeaker:BackgroundDancer;
	...
	override public function create()
	{
		...
		if (SONG == null)
			SONG = Song.loadFromJson('beginnings');
		...
		switch(SONG.song.toLowerCase())
		{
			case 'chiwld-season' | 'timber-terror' | '3-gnomes-in-a-trenchcoat': 
			{
				curStage = 'forest';

				var forest:FlxSprite = new FlxSprite(-40, 55).loadGraphic(Paths.image('forest/forestBG','week2'));
                forest.setGraphicSize(Std.int(forest.width * 0.9));
				forest.antialiasing = true;
				add(forest);

			}
			case 'ceramic-mischief': 
			{
				curStage = 'bully';

				var bully:FlxSprite = new FlxSprite(-40, 55).loadGraphic(Paths.image('bully/bullyBG','week3'));
                bully.setGraphicSize(Std.int(bully.width * 1.1));
				bully.antialiasing = true;
				add(bully);
			...
			case 'senpai' | 'roses':
			{
					...
					bgSpeaker = new BackgroundDancer(0, 0);
					bgSpeaker.scrollFactor.set(0.9, 0.9);
					bgSpeaker.updateHitbox();
					add(bgSpeaker);
			}
			case 'thorns':
			{
		...
		if (curStage == 'forest')
		{
			var overlay:FlxSprite = new FlxSprite(-40, 55).loadGraphic(Paths.image('forest/forestOverlay','week2'));
        ...
        noteSplashes = new FlxTypedGroup<FlxSprite>();
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if (SONG.song.toLowerCase() == 'timber-terror')
             gf.playAnim('scared');
  				 else
             gf.dance();
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var noteSplash:FlxSprite = new FlxSprite(0, strumLine.y - 80);
			noteSplash.frames = Paths.getSparrowAtlas('noteShock');
			noteSplash.antialiasing = true;
			switch (Math.abs(i))
			{
			//Heya, up ahead is some redundant looking code (with the press1 and press2 stuff). Initially it was programmed with the mindset of adding the week 7 gfx, but those aren't allowed on GB.
			case 0:
			noteSplash.x += Note.swagWidth * 0;
			noteSplash.animation.addByPrefix("press1", "Symbol 1 instance 1", 24, false);
			noteSplash.animation.addByPrefix("press2", "Symbol 1 instance 1", 24, false);
			case 1:
			noteSplash.x += Note.swagWidth * 1;
			noteSplash.animation.addByPrefix("press1", "Symbol 1 instance 2", 24, false);
			noteSplash.animation.addByPrefix("press2", "Symbol 1 instance 2", 24, false);
			case 2:
			noteSplash.x += Note.swagWidth * 2;
			noteSplash.animation.addByPrefix("press1", "Symbol 1 instance 3", 24, false);
			noteSplash.animation.addByPrefix("press2", "Symbol 1 instance 3", 24, false);
			case 3:
			noteSplash.x += Note.swagWidth * 3;
			noteSplash.animation.addByPrefix("press1", "Symbol 1 instance 4", 24, false);
			noteSplash.animation.addByPrefix("press2", "Symbol 1 instance 4", 24, false);
			}
			...
			noteSplash.animation.play('press' + FlxG.random.int(1,2));
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'chiwld-season' && healthBar.percent < 30)
        {
             gf.playAnim('scared');
             trace("SHE REAL SCARED THE BF ABOUT OT DIE (health)");
        }

        if (SONG.song.toLowerCase() == 'chiwld-season' && healthBar.percent > 60)
        {
         gf.playAnim('cheer');
         trace("She happy for bf :)");
        }

        		if (SONG.song.toLowerCase() == 'timber-terror' && healthBar.percent < 30)
        {
             gf.playAnim('scared');
             trace("OH LAWD");
        }
        ...
        if (SONG.song.toLowerCase() == 'timber-terror' && healthBar.percent > 30)
        {
         ...
         trace("OH LAWD LAWD");
        		...
        		if (SONG.song.toLowerCase() == '3-gnomes-in-a-trenchcoat' && healthBar.percent < 30)
        {
             gf.playAnim('scared');
             trace("SHE REAL SCARED THE BF ABOUT OT DIE (health)");
        }

        if (SONG.song.toLowerCase() == '3-gnomes-in-a-trenchcoat' && healthBar.percent > 60)
        {
         gf.playAnim('cheer');
         trace("She happy for bf :)");
        }
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				if (SONG.song.toLowerCase() == 'beginnings')
				{
					tweenCamIn();
				}
			}
			...
			switch(daRating)
			{
				...
				case 'sick':
					...
					if (FlxG.save.data.accuracyMod == 0)
						totalNotesHit += 1;
					sicks++;
				noteSplashes.members[daNote.noteData].visible = true;
				noteSplashes.members[daNote.noteData].animation.play('press' + FlxG.random.int(1,2),false,0);
			}
	...
	*/
	...
	override function stepHit()
	{
		...
		if (noteSplashes.length >= 4)
		{
		if (noteSplashes.members[0].animation.curAnim.finished && noteSplashes.members[0].visible)
			{
				noteSplashes.members[0].visible = false;
			}
		if (noteSplashes.members[1].animation.curAnim.finished && noteSplashes.members[1].visible)
			{
				noteSplashes.members[1].visible = false;
			}
		if (noteSplashes.members[2].animation.curAnim.finished && noteSplashes.members[2].visible)
			{
				noteSplashes.members[2].visible = false;
			}
		if (noteSplashes.members[3].animation.curAnim.finished && noteSplashes.members[3].visible)
			{
				noteSplashes.members[3].visible = false;
			}
		}
	...
	override function beatHit()
	{
		...
		if (curBeat % 16 == 15 && SONG.song == 'Beginnings' && dad.curCharacter == 'gf' && curBeat > 16 && curBeat < 48)
			{
				boyfriend.playAnim('hey', true);
